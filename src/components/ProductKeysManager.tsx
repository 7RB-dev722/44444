...
import { Plus, Trash2, Filter, Undo2, KeyRound, CheckCircle, XCircle, Copy, Search, DollarSign, TrendingUp } from 'lucide-react';
...
const ProductKeysManager: React.FC<ProductKeysManagerProps> = ({ products, keys, onKeysUpdate, saving, setSaving, setError, setSuccess }) => {
  ...
  const { productStats, totalSales, totalUsedKeys } = useMemo(() => {
    ...
    const sales = usedKeys.reduce((acc, key) => {
        const product = products.find(p => p.id === key.product_id);
        if (product) {
            const normalizedTitle = product.title.toLowerCase().trim();
            if (NET_PRICES[normalizedTitle]) {
                return acc + NET_PRICES[normalizedTitle];
            }
            // Fallback for products not in the net price list
            if (normalizedTitle.includes('codm')) {
                return acc + 31.0;
            }
        }
        return acc;
    ...
    return { productStats: stats, totalSales: sales, totalUsedKeys: usedKeys.length };
  }, [products, keys]);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
      ...
      <div className="lg:col-span-1 space-y-8">
        <div className="bg-slate-800 rounded-2xl p-6 border border-slate-700">
            <h4 className="text-lg font-bold text-cyan-400 mb-4">ملخص المبيعات</h4>
            <div className="space-y-3">
                <div className="flex justify-between items-center">
                    <span className="text-gray-400 text-sm flex items-center gap-2"><TrendingUp className="w-4 h-4"/> إجمالي المفاتيح المستخدمة</span>
                    <span className="font-bold text-white text-lg">{totalUsedKeys}</span>
                </div>
                <div className="flex justify-between items-center">
                    <span className="text-green-400 text-sm flex items-center gap-2"><DollarSign className="w-4 h-4"/> إجمالي المبيعات الصافية</span>
                    <span className="font-bold text-green-400 text-lg">${totalSales.toFixed(2)}</span>
                </div>
            </div>
        </div>
        <WithdrawalCalculator totalSales={totalSales} />
      </div>
    </div>
  );
};
